const apiKey = "f03cf729739d43fc9f5aee3979c110e0";
...
function displayRecipes(recipes) {
  ...
  recipes.forEach((recipe) => {
    const recipeCard = `
        ...
        <button class="btn btn-primary view-recipe-btn" data-id="${recipes.id}" data-toggle="modal" data-target="#recipeModal">View Recipe</button>
        </div>
        </div>
        </div>
        `;
...
function fetchRecipeDetails(recipeId) {
  const url = `https://api.spoonacular.com/recipes/${recipeId}/informations?apiKey=${apiKey}`;

  fetch(url)
    .then((response) => response.json())
    .then((data) => {
      displayRecipeDetails(data.results);
    })
    .catch((error) => {
